...
class DeliveryDateUpdater(ProductionPlanUpdater):
    ...
    async def update(self, input_data: Dict[str, Any]) -> None:
        ...
        original_program_planning = await ProgramPlanningRepository.get_by_week(original_week)
        if original_program_planning and "original_program_planning" in updated_programs:
            original_program_planning.production_runs = updated_programs["original_program_planning"].get("production_runs", [])
            await original_program_planning.save()
        
        # Update new program planning if week changed
        if new_week and new_week != original_week and "new_program_planning" in updated_programs:
            ...
            new_program_planning.production_runs = updated_programs["new_program_planning"].get("production_runs", [])
